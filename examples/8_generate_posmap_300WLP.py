''' 
Generate uv position map of 300W_LP.                                  # 在PRNet中，用来生成position map的ground truth
'''
import os, sys
import numpy as np
...
def run_posmap_300W_LP(bfm, image_path, mat_path, save_folder,  uv_h = 256, uv_w = 256, image_h = 256, image_w = 256):
    ...
    image_name = image_path.strip().split('/')[-1]                              # 读入图片
    image = io.imread(image_path)/255.
    [h, w, c] = image.shape

    info = sio.loadmat(mat_path)                                                # 读入对准参数
